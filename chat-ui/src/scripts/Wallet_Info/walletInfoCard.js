...
function showWalletInfoCard(response) {
    let messagesList = document.getElementById("messagesList");
    
    let walletDetails = `<div class="wallet-body"><div class="wallet-point">
                        <p class="card-text">Refund Points</p>
                        <h5 class="card-title">${response.totalBalance} Points</h5>
                        <p class="card-text">1 Point = 1 Rupee</p>
                    </div></div>`;
        ...
        response.data.map((item) => {
            ...
            tableRows += `<tr>
                            <td>
                                <div>
                                    <p>${transactionDate}</p>
                                    <p>Order id: ${item.orderDetails.dominosOrderNo}</p>
